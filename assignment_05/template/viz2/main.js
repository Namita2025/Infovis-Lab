import { state, onStateChange, setActiveYear, stopPlay } from '../main.js';

const MEDAL_COLORS = { Gold:'#FCB131', Silver:'#BFC5C9', Bronze:'#B87333' };
const CONTINENT_COLORS = { Europe:'#0085C7', Americas:'#DF0024', Asia:'#F4C300', Africa:'#00A651', Oceania:'#7B61A8' };
const MODES = { efficiency:'Medal Efficiency', tally:'Medal Tally', first:'First Achievement' };
...
function markup() {
  return `<div class="viz2-shell">
    ...
    <aside class="viz2-side" aria-label="Methodology and selected-country details">
      <button class="viz2-collapse" aria-label="Collapse details" aria-expanded="true">›</button>
      <h4>Method &amp; country</h4>
      <p class="viz2-method">Historical teams keep their original Olympic identity. Their data is drawn on a representative modern outline and explicitly labelled “Mapped to modern-day …”. Geography is not used to merge records.</p>
    ...
    <section class="viz2-controls-dock" aria-label="Map filters">
      <div class="viz2-controls-block">
        ...
        <div class="viz2-controls-grid">
          ...
          <div class="viz2-field"><label>Medals</label><div class="viz2-segments" data-v2-medal>${['All','Gold','Silver','Bronze'].map(x=>`<button data-value="${x}">${x}</button>`).join('')}</div></div>
        ...
        <div class="viz2-actions"><label class="viz2-check"><input type="checkbox" data-v2-pies checked> Show medal pies</label><button class="viz2-reset">Reset map</button></div>
      </div>
    </section>
...
export async function loadViz2() {
  ...
  svg.append('defs').html(`<filter id="viz2-glow"><feGaussianBlur stdDeviation="3" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>`);
  ...
  function render(){
    ...
    const color=d3.scaleSequential(d3.interpolateBlues).domain([0,cap]);
    countryG.selectAll('.country').attr('d',path).attr('tabindex',d=>getRecord(d)?'0':'-1').attr('aria-label',d=>{const r=getRecord(d);return r?`${r.label}, ${state.activeYear}`:null;})
      .classed('has-data',d=>!!getRecord(d)).classed('first-hit',d=>{const r=getRecord(d);return local.mode==='first'&&r&&firstMatch(r);}).classed('is-selected',d=>getRecord(d)?.noc===local.selected)
      .transition().duration(380).attr('fill',d=>{const r=getRecord(d);if(!r)return '#d7dbd9';if(local.mode==='first')return firstMatch(r)?'#0875d1':'#d6dde0';const v=metric(r);return Number.isFinite(v)?color(Math.min(v,cap)):'#d7dbd9';});
    ...
    root.querySelector('.viz2-legend').innerHTML=local.mode==='first'?`<strong>First achievement</strong><br>Blue = first ${local.first} in ${state.activeYear}`:`<strong>${MODES[local.mode]}</strong><div class="legend-ramp"></div>0 — ${local.mode==='efficiency'?cap.toFixed(2)+' medals / athlete':Math.ceil(cap)+' '+local.medal.toLowerCase()+' medals'}<br><span>Scale clipped at 95th percentile</span>`;
  ...
  function renderGlyphs(){
    ...
    const groups=glyphG.selectAll('.medal-glyph').data(local.pies?shown:[],d=>d.noc).join(enter=>{const g=enter.append('g').attr('class','medal-glyph');g.append('circle').attr('class','glyph-hit').attr('r',16);return g;},update=>update,exit=>exit.remove())
      .attr('transform',d=>{const feature=mapGeo.features.find(f=>f.properties.name===d.geometry_name);const p=feature&&projection(d3.geoCentroid(feature));return p?`translate(${p}) scale(${1/local.zoom})`:null;})
